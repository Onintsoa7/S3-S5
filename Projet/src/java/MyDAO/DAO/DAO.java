...
import MyDAO.database.Connector;
import MyDAO.utilities.MyParser;
...
public class DAO {
    ...
    public Object lastInsertedId(Connection con) throws Exception{
        ...
        String sql = "select * from "+tableName+" order by "+pkName+" DESC";
        
        if(this.getPk().getType()==String.class){
            sql = "SELECT * ,"+
        "CASE"+
        "  WHEN "+pkName+" ~ E'^\\\\d*$' THEN CAST("+pkName+" AS numeric) "+
        "ELSE CAST(regexp_replace("+pkName+", E'\\\\D', '', 'g') AS numeric) "+
        "END AS numeric_value "+
        " FROM "+tableName+" order by numeric_value desc";
    ...
    Object fillObject(ResultSet res) throws Exception{
        ...
        for (Field field : fields) {
                ...
                if (field.isAnnotationPresent(Column.class)) {
                    colinfo=field.getAnnotation(Column.class);
                    Object apidirina=res.getObject(colinfo.name());
                    Object databaseObject=null;
                    if (apidirina!=null) {
                        databaseObject=MyParser.parse(apidirina, field.getType());
                    }
